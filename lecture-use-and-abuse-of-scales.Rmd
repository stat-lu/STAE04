...
scales are vital in how we understand a visualization
...
```{r, echo = FALSE, fig.cap = "Change in reported crimes between 2019 and 2020 in Sweden (https://bra.se).", fig.show = "hold"}
bra2 <- tibble(
...
choosing limits for color-mapped variables can also be difficult

```{r, fig.cap = "Life expectancy in the US.", echo = FALSE, fig.width = 8, fig.height = 4.5}
library(maps)
library(mapproj)
...
```{r, fig.cap = "Life expectancy in the US with other limits.", echo = FALSE, fig.width = 8, fig.height = 4.5}
ggplot(states_data, aes(map_id = State)) +
  geom_map(aes(fill = `Life Exp`), map = states_map) +
  expand_limits(x = states_map$long, y = states_map$lat) +

...
For this course, please use the YAML block supplied in this template but
change `author`, `date`, and `title` fields. You can also remove the `references`
item in the YAML block.
...
but the simplest one is two simply create columns of text with dashes (`---`) separating
...
text inside brackets indicating the caption for the figure. Let's first download
an image to our working directory.
...
Footnotes can be useful to provide additional information^[If the footnote is short, it is 
often best to write it in-line, like this.]. For a longer footnote, it might be better to
...
To cite in R Markdown, you will need either 
1) a `.bib` file (with bibtex-formatted references) somewhere in your working directory 
or 2) a `references` field
in the YAML block; in this document we use the latter but only to keep this document
self-contained. Using a `.bib` file is recommended. Find the key of the reference you

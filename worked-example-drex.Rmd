...
the problem with boxplots: they can never "discover" anything but a single mode,
since boxplots are based on statistics only related to the quantiles of
the distribution.

### Dotplot

When studying a distribution, it is usually better to start off with a plot
with plot that does less abstraction, such as a
simply dotplot. The dots in a dotplot can represent more
...
```{r, fig.cap = "A density plots with various density estimate choices of the x variable."}
ggplot(xy, aes(x)) +
...
When you have several variables from the same dataset and thus related, it
often makes good sense to juxtapose them in a plot in order to make it
easier for the reader to compare the distributions. In the lecture,
we did you by first pivoting our dataset into a long format in order to have
one column that denotes the name of the variable and one column that
contains those variables' values. Let's do so here.
...
our distribution. Let's try a violin plot now.
...
can be used to patch together different ggplots. This is very useful if you
want to juxtapose plots that use different types of layers. Let's
...
together using `+`. Let's say we wanted to juxtapose a density plot of
variable `x` with a histogram of variably `y`---all we need to do is 
...
Simple, right? Patchwork has much more extensive functionality than this, 
however, so make sure to check out the
...
data. Stopping here, however, would unfortunately be a severe mistake. What
we've missed is to plot the *joint* distribution of these variables, which
is very easy to do using a scatter plot---which we will dig into during the

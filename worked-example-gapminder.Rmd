...
<iframe width="560" height="315" src="http://www.youtube.com/embed/Z8t4k0Q8e8Y?rel=0" frameborder="0" allowfullscreen></iframe>
...
ahead and install and load that package.
...
Just as we said in the introduction, this graph is not (yet) working out so well
for us. Let's make it animated instead. For this, we'll use
...
large_country_names <- 
  ...
  group_by(continent) %>%
  filter(pop >= sort(pop, decreasing = TRUE)[2]) %>%
  pull("country")
...
Then we put everything together; this time we also change the
...
## Interactive Gapminder Visualization

Interactive visualizations can be really effective, particularly when
...
Now we redraw the plot, adding a interactive slider to select the year using

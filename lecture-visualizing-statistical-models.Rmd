---
title: "Visualizing Statistical Models"
...
but if we're doing analysis we often also want **diagnostic**,
...
for `lm()` models usually get by fine with standard `plot()` method
...
.pull-right[
```{r ref.label = "residualplot", echo = FALSE, fig.height = 5.2}
```
]
...
(parametric) models estimate parameters: linear regression fits return **coefficients**

use `broom::tidy()` to extract summary statistics
...
it is easy to pipe the `broom::tidy()` output into ggplot2 to create coefficient
plots
...
augment new data with predictions from model using `broom::augment()`

...
the tidyverse approach. The first and easiest is to use `rename()`.
You use a `new_name = old_name` scheme to do so. Let's rename "vin" to 
"vintage".
...
practice for the same reason as it is unwise to include special characters
like `%`, `&`, `$`, and so on in variable names---it may cause some functions
to break unexpectedly.

You can also rename variables in your call to `select()` in the same fashion.

```{r}
us_cars %>%
  select(vintage = vin, vehicle_status = title_status)
```
...
end up with unexpected (and undesirable) consequences.

...
to work with spatial data in ggplot, we need to introduce a few other packages, such as

- [maps](https://CRAN.R-project.org/package=maps),
- [mapproj](https://CRAN.R-project.org/package=mapproj), and
- [ggmap](https://CRAN.R-project.org/package=ggmap).

it's typically a good idea to go for a minimalist theme when mapping
...
`map_data()` converts various objects to data that ggplot can use and also 
...
a possibility is just to plot the points
...
with point data, it is easy to add more layers
...
networks often come up in spatial data

visualizing networks using basic ggplot2 functionality is **complicated**, so we need to
use either `ggnetworkmap()`from [GGally](https://CRAN.R-project.org/package=GGally)
or the [ggnetwork](https://CRAN.R-project.org/package=ggnetwork) package

these functions requires a network to be built first; here we use data of airports
and flights in the US (see the source code for the rather complicated data set)
...
there exists a plethora of map projections, which all try to preserve some of
...
there is a plethora of packages in R that deal with spatial data, many with more

...
create using `matrix()`, `cbind()`, and `rbind()` functions
...
acess elemments, rows, and columns using `[` operator
...
like matrix, but columns may have different classes
...
a list of **any kind of objects** (of any dimensions)

```{r, echo = TRUE}
# use <name> = <element> syntax
x <- list(a = c("foo", "bar"), 
          b = matrix(1:4, 2, 2))
x
```
...
fun fact: a data frame is in fact a special type of list

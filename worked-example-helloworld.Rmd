...
other programming languages since before, you're notice that
...
So it's very easy to write an R program. You simply put some R code in
a text file with the ending `.R`, and then you can run that program
using a terminal, R Studio, or some other graphical user interface. So,
...
installed alongside R to make it easy to run scripts.] on it.

```{bash}
Rscript helloworld.R
```

This is just to showcase that R is really just a program that processes a 
text file in a specific way. The `.R` (and `.Rmd`) files that you will be
working with are only text and you don't need R to read them. You can just open 
...
Accessing a contiguous subvector is often desireable, in which case the `:` 
operator is a useful tool. It creates a vector of indices, like so:
...
after which all of that packages functions---and possibly also its 
...
particularly for filtering observations based on the values of a given variable.
